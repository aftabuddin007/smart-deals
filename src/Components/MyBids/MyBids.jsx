...
const [useBids,setUseBids] = useState([])
useEffect(()=>{
    if(user?.email){
...
fetch(`http://localhost:3000/bidData/${_id}`,{
    method:'DELETE'
})
.then(res=>res.json())
.then(data=>{
...
Swal.fire({
    ...
    const remainingBids = useBids.filter(bid=>bid._id !== _id)
    setUseBids(remainingBids)
    }
})
